...
export default function Confirm({ text, dialogRef, handleClickSuccessConfirm }: Props) {
  const handleCloseClick = () => {
    if (dialogRef.current) {
      dialogRef.current.close(); // 다이얼로그를 닫습니다.
    }

    // handleClickSuccessConfirm 콜백 함수가 존재하면 호출합니다.
  ...
  return (
    ...
    >
      <div className="md:w-[54rem] md:h-[25rem] w-[32.7rem] h-[22rem] bg-white">
        <p className="md:text-[1.8rem] text-[1.6rem] font-medium text-[#333236] absolute left-1/2 top-[calc(50%-2.1rem)] transform -translate-x-1/2">
          {text}
        </p>
